"""
RelatableFallbackEngine

When the main pipeline returns "no data found" because the user referenced
a value that doesn't exist in the dataset, this engine:

  1. Extracts the exact missing value from the hint message or the user query.
  2. Searches all string columns in the cached company DataFrame for similar values.
  3. Ranks candidates by text similarity.
  4. Substitutes rank-1 into the original query, runs it through the full pipeline,
     and returns the real result to the user.
  5. Asks: "reply 'no' for the next best match."

Confirmation logic (called before the normal pipeline on every message):
  - "no"          → advance to next candidate, repeat step 4-5.
  - "yes" / "ok"  → acknowledge and clear fallback state.
  - any other text → clear state; let the normal pipeline handle the new question.
"""
...
class RelatableFallbackEngine:
    ...
    async def _present(
        ...
        # Rewrite query and run the full normal pipeline with the substitute.
        # We snapshot the session history length beforehand and restore it after,
        # so the internal substitute-query call is invisible to the conversation
        # context. Without this, the LLM would see the rewritten query in history
        # instead of the user's actual question, breaking follow-up answers.
    ...
    def _extract_missing_info(
        ...
        """
        Return (missing_value, source_column).

        Strategy 1 — parse the build_empty_hint() message.
        This is the most reliable path because the hint already contains
        the exact column name and the value that failed the SQL WHERE filter.
        Format: "No data found for {col} = '{value}'. Available values..."

        Strategy 2 — scan the user query for tokens not present in any column.
        Used when strategy 1 finds nothing (generic hint or no WHERE clause).
        """
    ...
    def _find_missing_in_query(self, query: str, df: pd.DataFrame) -> tuple[str, str]:
        ...
        if not candidates:
            ...
            # Try bigrams (more specific) then single tokens, skipping stop words
            for size in (2, 1):
                for i in range(len(tokens) - size + 1):
                    phrase = " ".join(tokens[i: i + size])
                    if any(p.lower() in _STOP_WORDS for p in phrase.split()):
                        continue
                    if len(phrase) >= 2:
    ...
    def _infer_best_column(self, missing_value: str, query: str, df: pd.DataFrame) -> str:
        """
        Infer which column the missing value most likely belongs to.

        Strategy A — query-word match against column names (most reliable):
          The user's own words name the column they care about.
          e.g. "grade JJK" → query contains "grade" → column "Grade" is chosen.
          No hardcoding: any column name that overlaps with the query wins.

        Strategy B — column-name similarity to the missing value:
          Fallback when no column name appears in the query.
          Compares the missing value against column NAMES, not cell values,
          so "JJK" won't accidentally match an employee's name.
        """
        ...
        # Strategy A: column name tokens overlap with query tokens
        best_overlap_col = ""
        best_overlap = 0
        for col in obj_cols:
        ...
        # Strategy B: column name most similar to the missing value
    ...
    def _find_candidates(
        ...
        # ── Source column known: stay inside it, no threshold ─────────────────
        # If the hint told us the exact column (e.g. "Grade"), return only
        # alternatives from that column. No threshold — even a low-similarity
        # Grade value is more useful than a high-similarity Employee Name.
    ...
    def _similarity(self, a: str, b: str) -> float:
        ...
        Three signals:
          - SequenceMatcher ratio  (character-level edit distance)
          - Token Jaccard overlap  (word-level)
          - Character trigram cosine (sub-word)

        For short strings (≤5 chars, e.g. codes like "JKK", "L2") the sequence
        ratio dominates because trigrams are too sparse to be informative.
        """
        seq = SequenceMatcher(None, a.lower(), b.lower()).ratio()
        tok = self._token_overlap(a, b)
        cos = self._cosine_trigram(a, b)

        if max(len(a), len(b)) <= 5:
            score = 0.7 * seq + 0.3 * tok
        else:
            score = 0.4 * seq + 0.3 * tok + 0.3 * cos
    ...
    def _cosine_trigram(self, a: str, b: str) -> float:
